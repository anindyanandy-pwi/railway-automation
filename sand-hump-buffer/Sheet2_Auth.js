// ============================================================
//  Sheet2_Auth.gs — Sheet 2 ONLY
//  Run Sheet2_authorizeSheet1() ONCE to authorize access to
//  Sheet 1 and write Sheet 2 menu shortcut.
// ============================================================

var S2_SHEET1_ID = "1-EWkp8nq5aL_BKs3MQymexS-1gjrrXtPbLu1ZCZ1_FI";

function Sheet2_authorizeSheet1() {
  try {
    var ss1 = SpreadsheetApp.openById(S2_SHEET1_ID);
    console.log("Sheet 1 access confirmed: " + ss1.getName());
    try { SpreadsheetApp.getUi().alert(
      "Authorization successful!\n\n"+
      "Sheet 2 can now read/write to Sheet 1.\n\n"+
...
// ── Cross-sheet cache writer (Sheet 2 version) ────────────────────────────────
function s2_writeSummaryToSheet1Cache(scriptFn, summaryObj) {
  try {
    var ss1      = SpreadsheetApp.openById(S2_SHEET1_ID);
    var cacheTab = ss1.getSheetByName("_EXCEPTION_CACHE_");
    if (!cacheTab) {
      console.log("_EXCEPTION_CACHE_ tab not found in Sheet 1. Run setupAllSystemTabs() in Sheet 1 first.");
      return;
    }

    // Read SL config from Sheet 1
    ...
    var dateStr = s2_fmtDate(new Date());
    var jsonStr = JSON.stringify(summaryObj);
    var consec  = s2_calcConsecutive(cacheTab, scriptFn, summaryObj.totalExceptions);
    ...
    // Also update status dashboard in Sheet 1
    s2_updateSheet1Status(ss1, scriptFn, slNo, summaryObj.totalExceptions, ts);

    // Also write to weekly cache in Sheet 1
    s2_writeSheet1WeeklyCache(ss1, scriptFn, summaryObj);

  } catch(e) { console.log("s2_writeSummaryToSheet1Cache error: "+e.message); }
}

function s2_calcConsecutive(cacheTab, scriptFn, todayEx) {
  if (todayEx===0) return 0;
  try {
    var lr=cacheTab.getLastRow(); if (lr<2) return 1;
    var yesterday=s2_fmtDate(new Date(new Date().getTime()-86400000));
...
function s2_updateSheet1Status(ss1, scriptFn, slNo, exceptionsCount, ts) {
  try {
  ...
  } catch(e) { console.log("s2_updateSheet1Status error: "+e.message); }
}

function s2_writeSheet1WeeklyCache(ss1, scriptFn, summaryObj) {
  try {
    ...
    var weekStart=s2_getWeekStart(), jsonStr=JSON.stringify(summaryObj);
    var lr=Math.max(sh.getLastRow(),1), found=-1;
    if (lr>1) {
      var vals=sh.getRange(2,1,lr-1,8).getValues();
      for (var i=0;i<vals.length;i++) { if (vals[i][0]===scriptFn&&vals[i][7]===weekStart) { found=i+2; break; } }
    }
  ...
  } catch(e) { console.log("s2_writeSheet1WeeklyCache error: "+e.message); }
}

function s2_fmtDate(d) {
  return ('0'+d.getDate()).slice(-2)+'.'+('0'+(d.getMonth()+1)).slice(-2)+'.'+d.getFullYear();
}
function s2_getWeekStart() {
  var d=new Date(), day=d.getDay(), diff=(day===0)?-6:1-day;
  return s2_fmtDate(new Date(d.getFullYear(),d.getMonth(),d.getDate()+diff));
}

// ── AEN normalization (Sheet 2 version, same logic) ───────────────────────────
var S2_AEN_MAP = (function() {
...
function s2_normalizeAEN(raw) {
  if (!raw) return null;
  var s = String(raw).trim().replace(/\s*\([^)]*\)/g, '').trim();
  var lower = s.toLowerCase();
  if (S2_AEN_MAP[lower]) return S2_AEN_MAP[lower];
  var slashed = lower.replace(/\s+/g, '/');
  if (S2_AEN_MAP[slashed]) return S2_AEN_MAP[slashed];
  if (S2_AEN_MAP['aen/' + slashed]) return S2_AEN_MAP['aen/' + slashed];
  return null;
}

function s2_extractAENFromLabel(label) {
  var s = String(label||"");
  var m = s.match(/\(AEN:\s*([^)]+)\)/i);
  if (m) return s2_normalizeAEN(m[1].trim());
  m = s.match(/(?:^|\|\s*)AEN:\s*([^\|\(]+)/i);
  if (m) return s2_normalizeAEN(m[1].trim());
  return null;
}

function s2_buildSummary(exTypesObj) {
  var result={date:s2_fmtDate(new Date()),exceptions:{},totalExceptions:0};
  for (var exType in exTypesObj) {
    var items=exTypesObj[exType]||[];
    for (var i=0;i<items.length;i++) {
      var item=items[i];
      var label=typeof item==='string'?item:(item.label||String(item));
      var aen=s2_extractAENFromLabel(label)||"UNASSIGNED";
...
// ── Menu shortcut to Sheet 1 Daily Report ─────────────────────────────────────
function openSheet1DailyReport() {
  var url="https://docs.google.com/spreadsheets/d/"+S2_SHEET1_ID+"/edit#gid=855312642";
  try {
